...
# Frozen harness-test fixture order matching the TEST MapIds (6..11):
# test_field=6, test_town=7, ...  The engine indexes the test tables by
# (map_id - MAP_TEST_FIELD), so the emission order must be exactly this.
...
def emit_exits(levels_by_id):
    ...
    ordered_ids = [sid for sid in SCENE_ORDER + TEST_SCENE_ORDER if sid in levels_by_id]
...
def emit_c_code(levels_by_id, tilesets, bank=5):
...
def emit_actors_code(levels_by_id, bank=2):
    """Generate complete src/game/actors_content.c."""
    enemy_ids = load_enemy_types()
    ordered = [sid for sid in ACTOR_TABLE_ORDER + TEST_SCENE_ORDER if sid in levels_by_id]
    for sid in levels_by_id:
