 ...
 * src/battle/combo.c out of the fixed bank).  The fixed-bank wrapper
 * stages the target bank + logical address and its arguments into the
 * _DATA globals below, then calls banked_call_run() (asm) so the WRAM
 * trampoline never parses SDCC's stack layout.  The banked target reads
 * the staged globals.  g_bk_ptr_a/b carry pointer arguments; g_bk_byte_a/b
 * carry byte arguments.  The target must be self-contained (no calls back
 * into fixed-bank code). */
