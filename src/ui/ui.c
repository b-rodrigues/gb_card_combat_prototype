...
/* ── Player sprite ─────────────────────────────────────────────────
 * Hero frame 1 & 2 tile data is generated by tools/png2gb.py from
 ...
 * assets/card_frames.png; the array lives in bank 3 next to the renderer
 * that stamps the boxes). */
extern const uint8_t card_frame_tiles[];
...
void ui_init(void)
{
    ...
    /* Load the battle hand-card frame tiles (TL TM TR / L C R / BL BM BR)
     * and the turn-timer bar segments (filled, empty) from Bank 3
     * (generated card_frame_tiles.h, make gfx).  Sheet order: frames are
     * tiles 0-8 (VRAM 118-126), filled is tile 9 (VRAM 117), empty is
     * tile 10 (VRAM 127); tile 11 is a blank pad, never loaded.  Battle
     * enemy art starts at 128, so the 117-127 window is exclusive to
     * these once-loaded, never-clobbered tiles. */
    for (p = 0; p < 9; p++) {
        banked_copy(3, g_ui_screen_buf, card_frame_tiles + ((uint16_t)p << 4), 16);
        set_bkg_data((uint8_t)(UI_TILE_CARD_FRAME_BASE + p), 1, (const uint8_t *)g_ui_screen_buf);
    }
    banked_copy(3, g_ui_screen_buf, card_frame_tiles + (9u << 4), 16);
    set_bkg_data(UI_TILE_TIMER_FILLED, 1, (const uint8_t *)g_ui_screen_buf);
    banked_copy(3, g_ui_screen_buf, card_frame_tiles + (10u << 4), 16);
    set_bkg_data(UI_TILE_TIMER_EMPTY, 1, (const uint8_t *)g_ui_screen_buf);
...
{
    extern const uint8_t enemy_ow_tiles[];
    extern const uint8_t g_enemy_ow_tile_count;
    uint8_t ow_n, t, c;

    /* Hero overworld art: exactly 2 tiles = frames 1 & 2 (assets/
     * hero_sprites.png).  The old idle tile was loaded then overwritten
     * here; the player OAM slot now only ever shows frames 1 & 2 (see
     * ui_draw_actors_sprites). */
    set_sprite_data(HERO_DESOLATE_SPRITE_TILE_ID, 2, hero_desolate_sprite_tile);
    /* Shared per-enemy overworld sprites (assets/enemy_sprites.png blob in
     * bank 4): stream into OAM at ENEMY_OW_BASE in screen-buf chunks
     * (g_ui_screen_buf holds 128 bytes safely; no multiply anywhere). */
    banked_copy(4, &ow_n, &g_enemy_ow_tile_count, 1);
    for (t = 0; t < ow_n; t = (uint8_t)(t + 8)) {
        c = (uint8_t)(ow_n - t);
        if (c > 8) c = 8;
        banked_copy(4, (void *)g_ui_screen_buf,
                    (const void *)&enemy_ow_tiles[(uint16_t)t << 4],
                    (uint16_t)c << 4);
        set_sprite_data((uint8_t)(ENEMY_OW_BASE + t), c,
                        (const uint8_t *)g_ui_screen_buf);
    }
...
void ui_draw_actors_sprites(const World *world)
{
    ...
    /* Hero animates with frames 1 & 2 on every map (the idle tile is gone). */
...
/* Thin fixed-bank wrapper: the timer-bar draw body lives in bank 3
 * (ui_draw_battle_timer_banked, AGENTS.md 52.18 -- the fixed bank is hard
 * against 0x8000) and is dispatched through the banked-call trampoline.
 * The fill count is computed here and staged as a byte.  Called per
 * tile-boundary change from battle_screen.c and once at full draw. */
void ui_draw_battle_timer(const Battle *battle)
{
    if (!battle) return;
    g_bk_call_bank = 3;
    g_bk_call_target = (uint16_t)&ui_draw_battle_timer_banked;
    g_bk_byte_a = ui_calc_timer_bar(battle->timer_ticks);
    banked_call_run();
}
