...
void ui_init(void)
{
    ...
    for (p = 0; p < 64; p++) {
        uint8_t byte = ((const uint8_t *)cgb_palette)[p & 7];
...
static const char * const s_map_labels[5] = {
    "MAP: FIELD| HP:",
    "MAP: TOWN | HP:",
    "MAP: FORST| HP:",
    "MAP: MOUNT| HP:",
...
void ui_draw_overworld_hud(const World *world)
{
    ...
    label = (world->map_id <= MAP_CASTLE) ? s_map_labels[world->map_id] : s_map_labels[0];
...
static void ui_draw_battle_combo(const Battle *battle)
{
    ...
    for (i = 0; i < battle->combo_count; i++) {
        ui_draw_card_at((uint8_t)(7 + (i << 2)), 6, battle->hand[battle->selected_indices[i]]);
    }
}
...
void ui_update_battle(const Battle *battle)
{
    ...
    if (battle->result == BATTLE_RESULT_VICTORY) {
        action_msg = "VICTORY! PRESS [A]";
    } else if (battle->result == BATTLE_RESULT_DEFEAT) {
        action_msg = "DEFEATED! PRESS [A]";
    } else if (battle->result == BATTLE_RESULT_FLED) {
        action_msg = "FLED! PRESS [A]";
    } else if (battle->phase == BATTLE_PHASE_PLAYER_SELECT) {
